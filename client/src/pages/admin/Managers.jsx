...
import { localGallery } from "../public/localGallery";
...
export const headOfSchoolFields = [{ name: "title", label: "Page title" }, { name: "principalName", label: "Head of School name" }, { name: "principalQualification", label: "Qualification / title" }, { name: "principalImage", label: "Photo", type: "image" }, { name: "content", label: "Welcome message", type: "richtext" }, ...seoFields()];
...
export function GalleryManager() {
  ...
  async function importGallery() {
    try {
      ...
      window.localStorage.removeItem("rehoboth-api-cache:v1:gallery-categories-v4");
  ...
  useEffect(() => {
    if (checkedForImport.current) return;
    checkedForImport.current = true;

    async function importWhenEmpty() {
      try {
        const response = await http.get("/gallery");
        if (!response.data?.length) await importGallery();
      } catch (error) {
        toast.error(error.response?.data?.message || error.message || "Unable to load the gallery");
      }
    }
...
export function HeadOfSchoolManager() { return <ResourceManager title="Head of School Welcome" intro="Edit the leadership welcome page: page title, Head of School details, photo, welcome message, and search preview." endpoint="/pages/head-of-school-welcome" fields={headOfSchoolFields} singleton />; }
